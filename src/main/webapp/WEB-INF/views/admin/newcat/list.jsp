...
<body>
    <form action="<c:url value='/quantri/loai-bai-viet/danh-sach'/>" id="formSubmit" method="get">
        <h3 class="mt-4">Quản lý loại bài viết - ${model.totalItem } loại bài viết</h3>
                        ...
                        <div class="p-2 flex-grow-1 bd-highlight">
                            <div class="btn-toolbar" role="toolbar" aria-label="Toolbar with button groups">
                                <div class="btn-group mr-2" role="group" aria-label="First group">
                                    <a class="btn btn-primary" title='Thêm thể loại bài viết'
                                        href="<c:url value='/quantri/loai-bai-viet/chinh-sua'/>">
                                ...
                                <tbody>
                                    <c:forEach var="item" items="${model.listResult}">
                                        <tr>
                                            ...
                                            <td>${item.code}</td>
                                            <td style="text-align: center">
                                                <c:url var="updateNewCatURL" value="/quantri/loai-bai-viet/chinh-sua">
                                                    <c:param name="id" value="${item.id}"></c:param>
                                                </c:url>
                                                <a class="btn btn-primary btn-sm" title="Cập nhật thể loại bài viết"
                                                href='${updateNewCatURL}'>
        ...
        function deleteNewCat(data) {
            $.ajax({
                ...
                success: function (result) {
                    window.location.href = "${newCatURL}?page="+currentPage+"&message=delete_success";
                },
                error: function (error) {
                    window.location.href = "${newCatURL}?page=1&message=error_system";
                }
            });
        }
